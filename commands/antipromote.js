    ...
    try {
        ...
        if (!action) {
            const usage =
                ...
                `When enabled, any unauthorized promotion attempt will be instantly reversed.`;
        ...
        switch (action) {
            ...
            case 'enable':
                ...
                await sock.sendMessage(chatId, {
                    text: `*Antipromote Activated*\n\nUnauthorized promotions are now blocked!\nAny member promoted without permission will be instantly demoted back.`
                }, { quoted: createFakeContact(message) });
                break;
            ...
            case 'disable':
                ...
                await sock.sendMessage(chatId, {
                    text: `*Antipromote Deactivated*\n\nMembers can now be promoted normally.`
                }, { quoted: createFakeContact(message) });
                break;
        ...
        if (isBotAdmin) {
            ...
            await sock.sendMessage(chatId, {
                text: `*ANTIPROMOTE ACTIVE*\n\n` +
                      `@${authorPhone} tried to promote ${jids.length} member${plural}!\n\n` +
                      `Demoted back:\n${mentionLines}\n\n` +
