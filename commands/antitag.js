        ...
        switch (action) {
            case 'on':
                ...
                if (existingConfig?.enabled) {
                    await sock.sendMessage(chatId, { text: '*Antitag is already on*' }, { quoted: createFakeContact(message) });
                    return;
                }
            ...
            case 'off':
                ...
                await sock.sendMessage(chatId, { text: '*Antitag has been turned OFF*' }, { quoted: createFakeContact(message) });
                break;
            ...
            default:
                await sock.sendMessage(chatId, { text: `*Use ${prefix}antitag for usage.*` }, { quoted: createFakeContact(message) });
        }
    } catch (error) {
        console.error('Error in antitag command:', error);
        await sock.sendMessage(chatId, { text: '*Error processing antitag command*' }, { quoted: createFakeContact(message) });
    }
}
