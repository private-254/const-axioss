...
async function approveCommand(sock, chatId, message) {
    try {
        ...
        if (!chatId.endsWith('@g.us'))
            return sock.sendMessage(chatId, { text: '*Group only command.*'}, { quoted: createFakeContact(message) });
        ...
        catch { return sock.sendMessage(chatId, { text: '*Unable to fetch requests.*'}, { quoted: createFakeContact(message) }); }
        ...
        if (mentioned.length) {
            ...
            if (!valid.length)
                return sock.sendMessage(chatId, { text: `Not pending: ${invalid.map(j => '@' + j.split('@')[0]).join(', ')}`, mentions: invalid, quoted: message });
        ...
        return sock.sendMessage(chatId, { text: '*Unexpected error.*', quoted: message });
    }
}
