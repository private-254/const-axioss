...
async function antieditCommand(sock, chatId, message) {
    try {
        ...
        if (args === 'on') {
            ...
            replyText = `Anti‑edit protection is currently **${status}** for this chat.\n\nUse \`.antiedit on\` to enable or \`.antiedit off\` to disable.`;
        } else {
            // Invalid argument
            await sock.sendMessage(chatId, {
    ...
    sock.ev.on('messages.update', async (updates) => {
        for (const update of updates) {
            ...
            if (store && typeof store.loadMessage === 'function') {
                ...
                await sock.sendMessage(chatId, {
                    text: `A message was edited in this chat (original text not available).`
                }, { quoted: createFakeContact(message) });
                continue;
            }
            ...
            if (!originalMsg) {
                ...
                await sock.sendMessage(chatId, {
                    text: `A message was edited (could not fetch original).`
                }, { quoted: createFakeContact(message) });
                continue;
            }
            ...
            if (originalText && newText && originalText !== newText) {
                ...
                const notice = `*Message edited by ${sender}*\n\n*Original:*\n${originalText}\n\n*Edited to:*\n${newText}`;
                ...
                await sock.sendMessage(chatId, {
                    text: `A message was edited (changes detected).`
                }, { quoted: createFakeContact(message) });
            }
        }
    });
