...
async function addCommand(sock, chatId, message) {
  try {
    ...
    if (!chatId.endsWith('@g.us'))
      return sock.sendMessage(chatId, { text: "This command is for group only" }, { quoted: createFakeContact(message) });
    ...
    if (text) {
      ...
      return sock.sendMessage(chatId, { 
        text: "Usage: `.add 2547xxxxxxx` or reply to a user's message\n\n*Please provide a phone number* (e.g., .add 254.....)" 
      }, { quoted: createFakeContact(message) });
    }
    ...
    for (let r of res) {
      ...
      if (r.status === 409) {
        ...
        await sock.sendMessage(chatId, {
          text: `@${target.split('@')[0]} left recently.\nInvite link sent instead.`,
          mentions: [target]
        }, { quoted: createFakeContact(message) });
