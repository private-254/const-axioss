...
async function createGroupCommand(sock, chatId, senderId, message, rawText) {
    try {
        ...
        if (participants.length === 0) {
            await sock.sendMessage(chatId, {
                text: `❌ Please provide at least one participant number.\n\n` +
                      `Example: *.creategroup My Group | 255....*`
            }, { quoted: message });
            return;
        }
        ...
        if (err.message?.includes('bad-request')) {
            errText = `❌ Failed to create group: one or more numbers are not on WhatsApp or are invalid. Make sure all numbers include the country code (e.g. 255....).`;
        } else {
