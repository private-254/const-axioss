    ...
    try {
        ...
        if (!action) {
            const usage = `*Atidemote*\n\n` +
                ...
                `╭◆\n│ antidemote kick @user - Remove\n` +
                `│ antidemote ban @user - Ban\n` +
                `│ antidemote unban @user - Unban\n` +
        ...
        switch (action) {
            ...
            case 'get': {
                ...
                const statusText = `*Antidemote Status*\n\n` +
                    `╭◆Group: ${chatId.split('@')[0]}\n│` +
                    `Status: ${statusConfig.enabled ? 'ACTIVE' : 'INACTIVE'}\n│` +
                    `Protections: ${statusConfig.protectedCount || 0}\n│` +
            ...
            case 'kick': {
                ...
                try {
                    ...
                    await sock.sendMessage(chatId, { 
                        text: `@${cleanKick.split('@')[0]} has been removed from the group.\nKicked by: @${cleanSender.split('@')[0]}`,
            ...
            case 'ban': {
                ...
                if (alreadyBanned) {
                    await sock.sendMessage(chatId, { 
                        text: `@${cleanBan.split('@')[0]} is already banned.`,
                        mentions: [cleanBan]
                    }, { quoted: createFakeContact(message) });
                    return;
                }
            ...
            case 'history': {
                ...
                statsText += `│*Protections:* ${stats.protectedCount || 0}\n`;
                ...
                if (stats.recentKicks.length > 0) {
                    statsText += `*Recent Action (Last 5):*\n`;
                    stats.recentKicks.slice(0, 5).forEach((kick, i) => {
                        ...
                        const act = kick.reason === 'demote' ? 'Protected' : kick.reason === 'ban' ? 'Banned' : 'Kicked';
...
async function handleAntidemote(sock, chatId, participants, author) {
    try {
        ...
        if (isBotAdmin) {
            ...
            await sock.sendMessage(chatId, {
                text: `*ANTIDEMOTE ACTIVE*\n\n` +
                      `@${authorPhone} tried to demote ${jids.length} admin${plural}!\n\n` +
                      `Re-promoted back:\n${mentionLines}\n\n` +
