...
async function setAntilink(groupId, type, action) {
    try {
        ...
        if (type === 'allowed') {
            data.antilink[groupId].allowed = action;
        } else {
            data.antilink[groupId].enabled = type === 'on';
            data.antilink[groupId].action = action || 'delete';
        }
...
async function getAntilink(groupId, type) {
    try {
        ...
        if (type === 'allowed') {
            return data.antilink[groupId].allowed || [];
        }
...
async function isSudo(userId) {
    try {
        ...
        for (const sudoEntry of data.sudo) {
            if (compareJids(userId, sudoEntry)) {
                return true;
            }
        }
...
async function addSudo(userJid) {
    try {
        const { normalizeJid, compareJids } = require('./jid');
        const data = loadUserGroupData();
        if (!data.sudo) data.sudo = [];
        const normalized = normalizeJid(userJid);
        const alreadyExists = data.sudo.some(entry => compareJids(entry, normalized));
        if (!alreadyExists) {
...
async function removeSudo(userJid) {
    try {
        ...
        const data = loadUserGroupData();
        if (!data.sudo) data.sudo = [];
        const idx = data.sudo.findIndex(entry => compareJids(entry, userJid));
        if (idx !== -1) {
...
└┬❖ 「  {user}  」
   ...
   └───────────────┈ ⳹
      `,
            channelId: '120363408344756821@newsletter'
        };
